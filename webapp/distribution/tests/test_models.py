...
class ShiftModelTest(TestCase):
    ...
    def test_str_representation(self):
        ...
        shift = Shift.objects.create(order_round=self.order_round, start=time(14, 0), end=time(16, 0))
        # Format: date-start-end
        expected = "{}-{}-{}".format(self.order_round.collect_datetime.strftime("%Y-%m-%d"), "14:00", "16:00")
    ...
    def test_start_str_property(self):
        ...
        shift = Shift.objects.create(order_round=self.order_round, start=time(14, 30), end=time(16, 0))
        self.assertEqual(shift.start_str, "14:30")

    def test_end_str_property(self):
        """Test end_str property returns formatted time."""
        shift = Shift.objects.create(order_round=self.order_round, start=time(14, 0), end=time(16, 30))
    ...
    def test_ordering_by_start_time(self):
        ...
        shift1 = Shift.objects.create(order_round=self.order_round, start=time(16, 0), end=time(18, 0))
        shift2 = Shift.objects.create(order_round=self.order_round, start=time(14, 0), end=time(16, 0))

...
class RecentListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return ((None, _("Recent")), ("All", _("All")))
    ...
    def queryset(self, request, queryset):
        if self.value() is None:
            return queryset.filter(order_round__collect_datetime__gte=get_recent_date())
        elif self.value() == "All":
...
class ShiftAdmin(admin.ModelAdmin):
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == "members":
            kwargs["queryset"] = VokoUser.objects.filter(groups__id=config.DISTRIBUTION_GROUP).order_by(

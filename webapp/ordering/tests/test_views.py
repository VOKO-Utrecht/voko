...
class TestProductsView(VokoTestCase):
    def setUp(self):
        ...
        self.login()
        self.order = OrderFactory(paid=False, finalized=False,
                                  user=self.user, order_round=self.round)
    ...
    @skip("TODO: Update test, only suppliers for products in round are now returned")  # noqa
    ...
    def test_order_sold_out_product(self):
        ...
        ret = self.client.post(
            self.url,
            {"order-product-%d" % sold_out_odp.product.id: 1},
            follow=True
        )
        self.assertMsgInResponse(
            ret, "Het product '%s' van %s is uitverkocht!" %
                 (sold_out_odp.product.name,
                  sold_out_odp.product.supplier.name))

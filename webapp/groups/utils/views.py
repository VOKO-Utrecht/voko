...
class GroupmanagerFormView(FormView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs['group_pk'] = self.group_pk
        return kwargs

    def __init__(self, **kwargs) -> None:
        if (self.group_pk is None):
            raise ImproperlyConfigured("GroupmanagerFormView requires group primary key'")
        super().__init__(**kwargs)

    def post(self, request, *args, **kwargs):
        transport_group = Group.objects.get(pk=self.group_pk)
        form = GroupManagerForm(request.POST, group_pk=self.group_pk)
        if (form.is_valid()):

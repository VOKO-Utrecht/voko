email_confirm_mail = \
"""
Beste %(first_name)s,

Leuk dat je lid wilt worden van VOKO Utrecht!


Klik op onderstaande link om je aanmelding te bevestigen:

%(URL)s


Wanneer je de aanmelding hebt bevestigd zullen wij binnenkort contact met je opnemen over de eerstvolgende informatiebijeenkomst.


Groene groetjes,

VOKO Utrecht.
"""

user_enable_mail = \
"""
Beste %(first_name)s,

Leuk dat je onze informatieavond hebt bezocht!

Je kunt nu je account activeren.

Klik op onderstaande link om je account te activeren:

%(URL)s


Wanneer je je account hebt geactiveerd kun je inloggen en gebruikmaken van het bestelsysteem.


Groene groetjes,

VOKO Utrecht.
"""


password_reset_mail = \
"""
Beste %(first_name)s,

Er is een aanvraag voor een wachtwoordreset gedaan.

Klik op onderstaande link om een nieuw wachtwoord in te stellen:


%(URL)s


Als je geen wachtwoordreset hebt aangevraagd kun je deze mail negeren.


Groene groetjes,

VOKO Utrecht.
"""


eerste_bestelronde_mail_html = \
"""
Beste %(first_name)s,<br/><br/>

Zoals je weet gaan we nu echt van start met VOKO Utrecht! In dit bericht informeren we
je over de laatste ontwikkelingen en de eerste bestelronde:<br/><br/>

<strong>Indeling werkzaamheden</strong><br/>
We zijn na de eerste bijeenkomst druk geweest met het bestelsysteem, juridische zaken en natuurlijk
met het bijhouden van de ledenlijst en het indelen van nieuwe leden in de werkgroepen. Het leek ons het
handigst wanneer de eerste bestelronde wordt uitgevoerd door de initiatiefnemers
(Sarah, Lotte, Merel, Elma, Rik en Winny), dit omdat we dan weten hoeveel mensen waarvoor nodig zijn en
waar dingen fout gaan, maar ook omdat de coordinatoren van de werkgroepen dan de tijd hebben om
nieuwe leden voor de werkgroep goed te informeren. Tijdens de eerste ophaaldag zullen anderen dan
de initiatiefnemers daarom waarschijnlijk nog niet hoeven helpen, vanaf de tweede leverdag draait
in principe iedereen mee (al zal niet iedereen meteen aan de beurt zijn om te helpen). Iedereen die zich heeft
ingeschreven zal binnenkort bericht krijgen van de coordinator van een van de werkgroepen over de verdeling van
de werkzaamheden.<br/><br/>

<strong>Eerste leverronde</strong><br/>
De eerste bestellijst staat nu online!<br/><br/>

<u>! Let op !</u> Vanwege vertraging in de procedure bij de notaris hebben we helaas nog geen bankrekening kunnen
openen, hierdoor is het nog niet mogelijk de bestelling online te betalen. <strong>We vragen iedereen </strong>
daarom om tijdens de eerste bestelronde <strong>contant af te rekenen bij het ophalen</strong>, dit zal de kosten van je
eerste bestelling en van het lidmaatschap (20 euro) bedragen. Neem alsjeblieft het geld zo contant mogelijk mee.<br/><br/>

<strong>Hoe werkt het:</strong><br/>
Je kunt inloggen op <a href="http://leden.vokoutrecht.nl/">leden.vokoutrecht.nl</a>. Van daaruit klik je op "Producten".
Volg van daaruit de aanwijzingen!<br/><br/>

<strong>Belangrijke data:</strong><br/>
<ul>
<li>bestelronde open: maandag 15 september</li>
<li>bestelronde sluit: vrijdag 19 september, om 18.00 uur</li>
<li>ophaalmoment: woensdag 24 september, tussen 17.30 en 20.30 in cafe Averechts</li>
</ul>

<br/><br/>
<strong>Lidmaatschap nog niet bevestigd?</strong><br/>
Om te kunnen bestellen dien je je lidmaatschap te hebben bevestigd; doe dit dus als je het nog niet gedaan hebt.
Wanneer je hierover geen bericht van ons hebt ontvangen kan het zijn dat het in je spambox terecht is gekomen,
check die dus eerst. Indien je niets hebt ontvangen mail ons dan.<br/><br/>

Als je vragen hebt, mail ons dan gerust! Dit kan via info@vokoutrecht.nl.<br/><br/>

Veel plezier met bestellen en tot volgende week woensdag, we hebben er zin in!<br/><br/>

Groene groetjes,<br/>
VOKO Utrecht.
"""

eerste_bestelronde_mail_plain = \
"""
Beste %(first_name)s,

Zoals je weet gaan we nu echt van start met VOKO Utrecht! In dit bericht informeren we
je over de laatste ontwikkelingen en de eerste bestelronde:

## Indeling werkzaamheden ##
We zijn na de eerste bijeenkomst druk geweest met het bestelsysteem, juridische zaken en natuurlijk
met het bijhouden van de ledenlijst en het indelen van nieuwe leden in de werkgroepen. Het leek ons het
handigst wanneer de eerste bestelronde wordt uitgevoerd door de initiatiefnemers
(Sarah, Lotte, Merel, Elma, Rik en Winny), dit omdat we dan weten hoeveel mensen waarvoor nodig zijn en
waar dingen fout gaan, maar ook omdat de coordinatoren van de werkgroepen dan de tijd hebben om
nieuwe leden voor de werkgroep goed te informeren. Tijdens de eerste ophaaldag zullen anderen dan
de initiatiefnemers daarom waarschijnlijk nog niet hoeven helpen, vanaf de tweede leverdag draait
in principe iedereen mee (al zal niet iedereen meteen aan de beurt zijn om te helpen). Iedereen die zich heeft
ingeschreven zal binnenkort bericht krijgen van de coordinator van een van de werkgroepen over de verdeling van
de werkzaamheden.

## Eerste leverronde ##
De eerste bestellijst staat nu online!

## ! Let op ! ##
Vanwege vertraging in de procedure bij de notaris hebben we helaas nog geen bankrekening kunnen
openen, hierdoor is het nog niet mogelijk de bestelling online te betalen. We vragen iedereen
daarom om tijdens de eerste bestelronde contant af te rekenen bij het ophalen, dit zal de kosten van je
eerste bestelling en van het lidmaatschap (20 euro) bedragen. Neem alsjeblieft het geld zo contant mogelijk mee.

## Hoe werkt het: ##
Je kunt inloggen op http://leden.vokoutrecht.nl/ . Van daaruit klik je op "Producten".
Volg van daaruit de aanwijzingen!

## Belangrijke data: ##
* bestelronde open: maandag 15 september
* bestelronde sluit: vrijdag 19 september, om 18.00 uur
* ophaalmoment: woensdag 24 september, tussen 17.30 en 20.30 in cafe Averechts

## Lidmaatschap nog niet bevestigd? ##
Om te kunnen bestellen dien je je lidmaatschap te hebben bevestigd; doe dit dus als je het nog niet gedaan hebt.
Wanneer je hierover geen bericht van ons hebt ontvangen kan het zijn dat het in je spambox terecht is gekomen,
check die dus eerst. Indien je niets hebt ontvangen mail ons dan.

Als je vragen hebt, mail ons dan gerust! Dit kan via info@vokoutrecht.nl.

Veel plezier met bestellen en tot volgende week woensdag, we hebben er zin in!

Groene groetjes,
VOKO Utrecht.
"""

order_reminder_mail = \
"""
Beste %(first_name)s,

Hierbij willen we je er even aan herinneren dat de bestelronde van VOKO Utrecht vanavond sluit om 18.00 uur,
je hebt dus tot dan om je bestelling af te ronden en te bevestigen.

We vragen je om de betaling voor deze eerste bestelronde contant te voldoen tijdens het ophaalmoment, 
dat zal plaatsvinden op woensdag 8 oktober, van 17.30 - 19.30 uur.

Groeten,
VOKO Utrecht
"""

tweede_bestelronde_mail_plain = \
"""
Beste %(first_name)s,


De tweede bestellijst staat nu online!

## ! Let op ! ##
Ten eerste: Vanwege vertraging in de procedure bij de notaris hebben we helaas nog geen bankrekening kunnen
openen, hierdoor is het nog niet mogelijk de bestelling online te betalen. We vragen iedereen
daarom om ook tijdens de tweede bestelronde contant af te rekenen bij het ophalen, dit zal de kosten van je
bestelling en eventueel (wanneer je voor het eerst bestelt) van het lidmaatschap (20 euro) bedragen.
Neem alsjeblieft het geld zo contant mogelijk mee.

Ten tweede: In verband met een andere activiteit in Cafe Averechts zal de uitdeelavond eenmalig tot 19:30 duren (in plaats van tot 20.30).

## Hoe werkt het: ##
Je kunt inloggen op http://leden.vokoutrecht.nl/ . Van daaruit klik je op "Producten".
Volg van daaruit de aanwijzingen!
Voor de zekerheid: vergeet niet je bestelling te bevestigen! Je bestelling is pas definitief wanneer je een bevestiging per e-mail hebt ontvangen.

## Belangrijke data: ##
* bestelronde open: maandag 29 september
* bestelronde sluit: vrijdag 3 oktober, om 19.00 uur
* ophaalmoment: woensdag 8 oktober, tussen 17.30 en **19.30** in cafe Averechts

## Lidmaatschap nog niet bevestigd? ##
Om te kunnen bestellen dien je je lidmaatschap te hebben bevestigd; doe dit dus als je het nog niet gedaan hebt.
Wanneer je hierover geen bericht van ons hebt ontvangen kan het zijn dat het in je spambox terecht is gekomen,
check die dus eerst. Indien je niets hebt ontvangen mail ons dan.

Als je vragen hebt, mail ons dan gerust! Dit kan via info@vokoutrecht.nl.

Veel plezier met bestellen en tot volgende week woensdag!

Groene groetjes,
VOKO Utrecht.
"""